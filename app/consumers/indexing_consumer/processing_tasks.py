...
def process_face_image(vector_store_actor, collection_id: str, 
    ...
    try:
        ...
        if _FACE_SERVICE_CACHE is None:
            logger.debug(f"Initializing face service for worker process (first time)")
        ...
        logger.info(f"Detected {face_count} faces in image {image_id} using CPU {cpu_num}")
        ...
        logger.error(f"Error in Ray task while processing image {image_id}: {str(e)}")
        raise 