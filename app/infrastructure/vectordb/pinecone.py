...
class PineconeMetadata(BaseModel):
    face_detection_id: str = Field(
        description="External system identifier for the face detection"
    )
    image_id: str = Field(
        description="External system identifier for the image"
    )
    collection_id: str = Field(
        description="External system identifier for the collection"
    )
    confidence: float = Field(
        description="Confidence score of the face detection"
    )
...
class PineconeVectorStore(VectorStore):
    ...
    async def store_face(
        ...
        """Store a face embedding in an collection.
    ...
    async def search_faces(
        ...
        try:
            ...
            # Query Pinecone with metadata filter for collection
            ...
            for match in results.matches:
                ...
                face = Face(
                    ...
                    embedding=match.values if hasattr(
                        match, 'values') else None
                )
    ...
    async def delete_face(
        ...
        """Delete face embeddings for an image from a collection."""
        try:
